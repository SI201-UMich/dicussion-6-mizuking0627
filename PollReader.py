...
class PollReader():
    ...
    def build_data_dict(self):
        ...
        # iterate through each row of the data, skipping the header row [FIXED]
        for row in self.raw_data[1:]:

            # split up the row by column, using a comma as the separator [FIXED]
            separated = row.strip().split(',')

            # The 'sample' column contains size and type, so we need to split it
            sample_info = separated[2].split(' ')
            sample_size = int(sample_info[0])
            sample_type = sample_info[1]

            # map each part of the row to the correct column [FIXED]
    ...
    def likely_voter_polling_average(self):
        ...
        harris_avg = harris_total / lv_count
        trump_avg = trump_total / lv_count
    ...
    def polling_history_change(self):
        ...
        # The data is in reverse chronological order (latest first).
        # "Latest 30 polls" are the first 30 in the list.
        latest_harris_avg = sum(harris_results[:30]) / 30
        latest_trump_avg = sum(trump_results[:30]) / 30

        # "Earliest 30 polls" are the last 30 in the list.
        earliest_harris_avg = sum(harris_results[-30:]) / 30
        earliest_trump_avg = sum(trump_results[-30:]) / 30

        # Calculate the net change (latest - earliest)
